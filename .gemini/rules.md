...
- ユーザーに提案する場合は、一つずつテキストベースでコピペできるように出力すること。
- 複数のコミット名を提案する場合は、ユーザーが指示を選びやすいように必ず連番（1, 2, 3... 等）を振って提示すること。
...
- 配布ファイルを作成する際は、動作に必要なファイルを集めてフォルダにまとめ、ZIPで圧縮してルートフォルダに置くこと。
...
### セキュリティとプライバシー（個人情報の保護）

- ドキュメント、スクリプト、および出力結果には、**個人環境に依存する情報（フルパス、OSのユーザー名など）を絶対に含めない**こと。
- パスを指定する場合は必ず **相対パス**（例: `./` やプロジェクトディレクトリ起点）を使用すること。
- その他エッジケースとして、OSの特定の環境変数、秘密鍵、APIキー、ローカルIPアドレス、MACアドレス、クリップボードの内容など、個人情報や機密情報と見なされ得る一切の情報をいかなる形でも含めない・記録しないよう徹底すること。
